...
class ContentBlockDeltaEventHandler implements BedrockStreamEventHandler {
  ...
  handle(
    ...
    return ContentBlockDelta.visit(deltaEvent.delta, {
      text: (text) => {
        // contentBlockIndex 1 表示正常文本内容
        if (deltaEvent.contentBlockIndex === 1) {
      ...
      toolUse: (toolUseDelta: ToolUseBlockDelta) => {
        if (toolUseDelta.input && state.toolCalls.length > 0) {
...
class MetadataEventHandler implements BedrockStreamEventHandler {
  ...
  handle(
    ...
    const metadataEvent = event as ConverseStreamMetadataEvent
  ...
  private finalizeResponse(
    ...
    if (state.stopReason === 'tool_use' && state.toolCalls.length > 0) {
      // 处理工具调用
      const processedToolCalls = this.processToolCalls(state.toolCalls)
      controller.enqueue({
      ...
      // 发送使用统计
      this.emitUsageStatistics(controller, state.usage)
    }
  }
   ...
   * 处理工具调用数据
   */
  private processToolCalls(toolCalls: BedrockSdkToolCall[]): BedrockSdkToolCall[] {
   ...
   * 发送使用统计信息
   */
  private emitUsageStatistics(controller: TransformStreamDefaultController<GenericChunk>, usage: any): void {
    controller.enqueue({
      ...
      controller.enqueue({
        ...
        response: {
          usage: {
            ...
            total_tokens: state.usage.totalTokens || 0
          }
        }
      })
    }
...
> {
  ...
  override buildSdkMessages(
    ...
    if (this.isConverseCommandOutput(output)) {
      ...
      if (hasToolCalls) {
        const thinkingText =
          this.lastThinkingBlock?.thinking ||
          (typeof output === 'string' && output.trim() ? output : '我需要使用工具来帮助处理这个请求。')

        // 注意：思考内容在Bedrock中需要特殊处理，这里暂时使用文本块
        assistantContent.push({ text: `<thinking>${thinkingText}</thinking>` })
